...
Stream这个抽象提供流一个一致的接口，能够进行读/写数据。
...
C++提供了模版来解决这类问题，即可以将类型的绑定推迟到编译时，使得在编写代码时能够不直接将类型写死，大大提高了代码编写的灵活性。
...
上面的Vector定义其实有一个问题，那就是不能通过 `at` 函数来修改元素的值。不过我们可以通过重载 `at` 来解决这一问题：
...
在C++20中提供的解决方案就是`Concept`，所谓concept，**本质上就是对模版参数的限定**，避免在模版实例化后出现问题
...
- 内存开销大，创建了两个临时的std::vector，如果数据量很大的话，会消耗大量内存
- CPU开销大，数据被完整地遍历了两遍
